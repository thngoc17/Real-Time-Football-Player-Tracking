...
class JerseyColorDataset(Dataset):
    """Custom dataset for jersey color classification"""
...
class JerseyColorClassifier:
    """Configurable jersey color classifier"""

    def __init__(self, config, num_classes, outputs_dir, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.device = device
        ...
        self.criterion = self._create_criterion(config['training']['loss'])
    ...
    def _create_criterion(self, loss_config):
    ...
    def train(self, train_loader, val_loader, num_epochs=20, save_path='jersey_color_model.pth'):
        """Train the model"""
    ...
    def test(self, test_loader, model_path='jersey_color_model.pth'):
        """Test the model"""
        # Load best model
    ...
    def plot_training_history(self):
        ...
        plt.tight_layout()
        training_history_path = self.outputs_dir / 'training_history.png'
    ...
    def plot_confusion_matrix(self, cm, class_names):
        ...
        plt.title('Confusion Matrix - Jersey Color Classification')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.xticks(rotation=45)
        plt.yticks(rotation=0)
        plt.tight_layout()
        confusion_matrix_path = self.outputs_dir / 'confusion_matrix.png'
...
def create_default_config(config_path):
    """Create a default train_color.yaml configuration file"""
    default_config = {
        'model': {
            'name': 'resnet50',
            'pretrained': True
...
def main():
    parser = argparse.ArgumentParser(description='Jersey Color Classification with YAML Configuration')
    parser.add_argument('--config', type=str, default=None,
                        help='Path to YAML configuration file (if not provided, will use root/config/train_color.yaml)')
    ...
    if args.config is None:
        args.config = str(config_dir / "train_color.yaml")
    ...
    # Set paths
    data_path = str(data_dir / "color_dataset")
    model_save_path = str(outputs_dir / "jersey_color_model.pth")
    ...
    # Create datasets
    train_dataset = JerseyColorDataset(data_path, 'train', train_transform)
    val_dataset = JerseyColorDataset(data_path, 'val', val_test_transform)
    test_dataset = JerseyColorDataset(data_path, 'test', val_test_transform)
    ...
    classifier = JerseyColorClassifier(config, num_classes, outputs_dir)
    ...
    if args.test_only:
        ...
        results_json_path = outputs_dir / 'training_results.json'
        with open(results_json_path, 'w') as f:
            json.dump(results, f, indent=2)

        # Save config used for training
        config_save_path = outputs_dir / 'training_config.yaml'
